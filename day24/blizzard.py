    ...
    with open(file_name, "r") as f:
        for yy, line in enumerate(f):
            ...
            if 0 == yy:
                ...
                # One above the goal, because there are no winds in the goal
                goal = (dot_index(line), yy - 1)
                width = len(line) - 2
                height = yy - 1
            else:
...
def find_new_possible_pos(possible_positions: Set[Tuple[int, int]], blizzards: List[Set[Tuple[int, int]]], height: int, width: int) -> Set[Tuple[int, int]]:
    ...
    for pos in possible_positions:
        ...
        if old_x < width and old_y > 0 and not windy((old_x + 1, old_y), blizzards):
        ...
        if old_x > 1 and old_y > 0 and not windy((old_x - 1, old_y), blizzards):
...
def main():
    ...
    while True:
        ...
        round += 1
        if goal in possible_positions:
            break
    # Add one because we found position above goal
    print(round + 1)
