...
def main():
    ...
    print(len(wind_pattern))
    # num_rocks = 4 * len(SHAPE_LIST) * len(wind_pattern)
    num_rocks = 10000
    wind_index = 0
    five_group_heights = []
    for ii in range(num_rocks):
        ...
        while move_rock_down(room, rock_height, rock_top_row):
            ...
            wind_index += 1
        if ii % 5 == 4:
            five_group_heights.append(len(room) - sum(five_group_heights))
    print(five_group_heights)
    print(sum(five_group_heights))
    print(25*"-")
    print_room(room[-5:])
    print(len(room))
