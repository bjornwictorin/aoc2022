...
def main():
    ...
    free_disk_space = total_disk_space - max(dir_sizes)
    print(f"free space: {free_disk_space}")
    needed_disk_space = required_size - free_disk_space
    selected_dir_sizes = [dir_size for dir_size in dir_sizes if dir_size >= needed_disk_space]
    assert (min(selected_dir_sizes) + free_disk_space >= required_size)
    print(min(selected_dir_sizes))
